...
export default async function CustomerResultPage() {
  ...
  if (!customer) return <main className="private-result-page"><section className="result-unavailable"><p>Civilon Price Check</p><h1>This Price Check result link is no longer available.</h1><p>For privacy, Civilon cannot provide additional information about this link.</p><Link href="/contact-us">Contact Civilon</Link></section></main>;
  ...
  return <main className="private-result-page"><CustomerResultView model={model} action={<ResultSourcingAction alreadyRequested={customer.sourcingRequested} />} /></main>;
}
