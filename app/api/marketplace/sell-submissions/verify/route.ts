...
import {
  SELL_SUBMISSION_VERIFY_MAX_BODY_BYTES,
 ...
 * one from a real submission. Only the `status` field differs, and it has
 * exactly two values: a repeat confirmation by the legitimate holder reports
 * "verified" just as the first one did.
 */
function verificationResponse(status: "verified" | "unavailable") {
  const body: SellSubmissionVerifyResponse = status === "verified"
    ? { ok: true, status }
    : { ok: false, status };
 ...
 * supplier ever saw the page — and would put the credential in a query string,
 * where it survives in access logs. Neither is acceptable for a credential that
 * activates a submission, so the only mutating method here is POST, and an
 * Origin header from an approved Civilon host is required.
 */
export async function POST(request: Request) {
  if (!isSellSubmissionEnabled()) {
  ...
  if (new TextEncoder().encode(rawBody).byteLength > SELL_SUBMISSION_VERIFY_MAX_BODY_BYTES) {
    return verificationResponse("unavailable");
  }
  ...
  // supplier out of their own link.
  const limit = consumeMarketplaceAttempt(
    marketplaceRateLimitKey(`sell-submission-verify:${network}:${token.slice(0, 12)}`),
  );
  if (!limit.allowed) return verificationResponse("unavailable");

  try {
    const [{ priceCheckDb }, { verifySellSubmissionContact }] = await Promise.all([
      import("@/db/price-check"),
      import("@/lib/marketplace/sell-verification-service"),
    ]);
    const result = await verifySellSubmissionContact(priceCheckDb, { token });
