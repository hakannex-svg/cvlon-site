...
export default async function PriceCheckDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AdminChrome user={access.user} active="queue">
    <section className="admin-page admin-detail-page">
      ...
      {priceCheck.status === "quote_requested" && <section className="admin-quote-followup" aria-labelledby="quote-followup-title"><div><p className="admin-eyebrow">Delivery & follow-up</p><h2 id="quote-followup-title">Customer requested a Civilon quote</h2><p>The customer requested sourcing assistance from this Price Check. Follow the linked sourcing opportunity.</p></div><a href="#result">Review sourcing status ↓</a></section>}
