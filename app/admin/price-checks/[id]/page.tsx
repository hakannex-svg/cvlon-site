...
import { AdminWorkflowOverview } from "@/components/admin/AdminWorkflowOverview";
...
export default async function PriceCheckDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const jobsNeedAttention = jobs.some(job => job.state === "failed" || job.state === "dead_letter");
  ...
  return <AdminChrome user={access.user} active="queue">
    <section className="admin-page admin-detail-page">
      ...
      <AdminWorkflowOverview status={priceCheck.status} assigned={Boolean(priceCheck.assignedAdminUserId)} />
      {priceCheck.status === "quote_requested" && <section className="admin-quote-followup" aria-labelledby="quote-followup-title"><div><p className="admin-eyebrow">Delivery & follow-up</p><h2 id="quote-followup-title">Customer created a Civilon Buy Request</h2><p>{resultData.linkedBuyRequest ? `Buy Request ${resultData.linkedBuyRequest.publicReference} is linked to this exact Price Check result.` : "The customer requested sourcing assistance before the full Buy Request conversion was available."}</p></div>{resultData.linkedBuyRequest ? <a href={`/admin/buy-requests/${resultData.linkedBuyRequest.id}`}>Open Buy Request →</a> : <a href="#result">Review sourcing status ↓</a>}</section>}
      <nav className="admin-section-nav" aria-label="Request sections"><a href="#overview">Overview</a><a href="#documents">Documents</a><a href="#review">Review</a><a href="#comparables">Comparables</a><a href="#result">Result</a><a href="#activity">Activity</a></nav>

      <div className="admin-detail-grid">
        <div className="admin-detail-main">
          <section id="overview" className="admin-phase-group" aria-labelledby="phase-intake-title"><header className="admin-phase-heading"><p className="admin-eyebrow">Phase 1 · Intake & ownership</p><h2 id="phase-intake-title">Understand the request</h2><p>Confirm who submitted the request, what they entered, and who owns the next operational step.</p></header>
          <section className="admin-panel" aria-labelledby="requester-heading"><div className="admin-panel-heading"><div><p className="admin-eyebrow">Customer & contact</p><h2 id="requester-heading">Requester</h2><p>Who submitted this request and how Civilon may contact them about this service.</p></div></div><dl className="admin-definition-grid">
          ...
          <section className="admin-panel" aria-labelledby="original-heading"><div className="admin-panel-heading"><div><p className="admin-eyebrow">Customer submission · Read-only</p><h2 id="original-heading">Original submission</h2><p>This is the transaction information originally submitted by the customer. It cannot be edited.</p></div><span>Revision 1</span></div><dl className="admin-definition-grid">
          ...
          <section id="documents" className="admin-phase-group" aria-labelledby="phase-verify-title"><header className="admin-phase-heading"><p className="admin-eyebrow">Phase 2 · Verify transaction</p><h2 id="phase-verify-title">Confirm the transaction record</h2><p>Review clean source documents, verify any extraction proposal, and create a documented revision when the reviewed facts differ.</p></header>
          <section className="admin-panel" aria-labelledby="docs-heading"><div className="admin-panel-heading"><div><p className="admin-eyebrow">Source requirements</p><h2 id="docs-heading">Documentation requirements</h2><p>Use these customer-selected requirements when reviewing the supporting documents.</p></div><a className="admin-guide-link" href="/admin/help#extraction">Guide →</a></div>{documentation.length ? <ul className="admin-chip-list">{documentation.map(item => <li key={item.requirementCode}>{item.requirementCode.replaceAll("_", " ")}{item.otherText ? ` — ${item.otherText}` : ""}</li>)}</ul> : <p className="admin-muted">No documentation requirements were selected.</p>}</section>
          ...
          <section id="review" className="admin-panel" aria-labelledby="reviewed-heading"><div className="admin-panel-heading"><div><p className="admin-eyebrow">Confirmed / reviewed transaction</p><h2 id="reviewed-heading" tabIndex={-1}>Reviewed transaction</h2><p>This is the transaction version Civilon will use for analysis. Create a documented revision when customer or document data needs correction.</p></div>{reviewedSnapshot && <span>Revision {latestRevision.version}</span>}</div>{reviewedSnapshot ? <><dl className="admin-definition-grid">{Object.entries(reviewedSnapshot).filter(([key]) => key !== "documentationCodes").map(([key, item]) => <div key={key}><dt>{key.replace(/([A-Z])/g, " $1")}</dt><dd>{value(item)}</dd></div>)}</dl><p className="admin-revision-reason"><b>Change reason:</b> {latestRevision.changeReason}</p></> : <div className="admin-empty-state"><strong>No staff correction has been created.</strong><p>Analysis will use the original submitted transaction until an authorized revision is saved.</p></div>}</section></section>

          <section id="comparables" className="admin-phase-group" aria-labelledby="phase-analysis-title"><header className="admin-phase-heading"><p className="admin-eyebrow">Phase 3 · Analyze evidence</p><h2 id="phase-analysis-title">Select evidence and calculate indicators</h2><p>Choose only relevant governed observations. Civilon’s deterministic engine calculates the observed indicators; AI does not calculate the range.</p><a className="admin-guide-link" href="/admin/help#comparables">Comparable guide →</a></header>
          ...
          <section id="result" className="admin-phase-group" aria-labelledby="phase-result-title"><header className="admin-phase-heading"><p className="admin-eyebrow">Phase 4 · Prepare result</p><h2 id="phase-result-title">Review the customer-facing result</h2><p>AI explanation is optional drafting assistance. Approval and sending remain separate human-controlled actions.</p><a className="admin-guide-link" href="/admin/help#result">Result guide →</a></header>
          ...
          /> : <section className="admin-panel admin-placeholder"><p className="admin-eyebrow">Customer result</p><h2>No customer result can be drafted yet.</h2><p>A persisted deterministic analysis must be marked analysis ready before customer-result preparation begins.</p></section>}</section>

          <section id="activity" className="admin-phase-group" aria-labelledby="phase-delivery-title"><header className="admin-phase-heading"><p className="admin-eyebrow">Phase 5 · Delivery & follow-up</p><h2 id="phase-delivery-title">Confirm delivery and preserve the record</h2><p>Monitor secure delivery, sourcing follow-up, and the append-only operational history.</p></header>
          <details className="admin-panel admin-history-disclosure"><summary><span><b id="audit-heading">Audit timeline</b><small>Append-only activity history · {audit.length} events</small></span></summary><ol className="admin-timeline" aria-labelledby="audit-heading">{audit.map(event => <li key={event.id}><time>{formatDateTime(event.createdAt)}</time><strong>{event.action.replaceAll("_", " ").replaceAll(".", " ")}</strong><span>{event.afterVersionReference ?? event.beforeVersionReference ?? "Recorded"}</span></li>)}</ol></details></section>
        </div>
        <section id="actions" className="admin-detail-aside" aria-label="Administration actions">
          <header className="admin-aside-heading"><p className="admin-eyebrow">Next action</p><h2>Request controls</h2><p>Use only the actions permitted for your role and the persisted workflow state.</p></header>
          ...
          {jobsNeedAttention ? <section className="admin-panel admin-processing"><p className="admin-eyebrow">Processing · Attention required</p><h2>Job visibility</h2>{jobs.length ? <ul>{jobs.map(job => <li key={job.id}><b>{job.jobType}</b><span>{job.state}</span>{job.sanitizedErrorCode && <small>{job.sanitizedErrorCode}</small>}</li>)}</ul> : <p className="admin-muted">No processing jobs are associated with this request.</p>}</section> : <details className="admin-panel admin-history-disclosure admin-processing"><summary><span><b>Processing / jobs</b><small>{jobs.length ? `${jobs.length} completed or informational jobs` : "No processing jobs"}</small></span></summary>{jobs.length ? <ul>{jobs.map(job => <li key={job.id}><b>{job.jobType}</b><span>{job.state}</span>{job.sanitizedErrorCode && <small>{job.sanitizedErrorCode}</small>}</li>)}</ul> : <p className="admin-muted">No processing jobs are associated with this request.</p>}</details>}
        </section>
      </div>
    </section>
  </AdminChrome>;
