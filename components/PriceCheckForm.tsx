...
const initialValues: FormValues = {
  partNumber: "", quantity: "1", quoteOrPurchased: "quote", transactionType: "outright",
  conditionCode: "SV", unitPrice: "", currencyCode: "USD", aog: false,
...
const transactionFields = new Set([
  "partNumber", "quantity", "quoteOrPurchased", "transactionType", "conditionCode",
  "unitPrice", "currencyCode", "aog", "description", "aircraftModel", "coreCharge",
...
export function PriceCheckForm() {
  ...
  function presentServerErrors(nextErrors: Record<string, string>) {
    ...
    if (fields.some((field) => transactionFields.has(field))) setStep(1);
    else if (fields.some((field) => documentFields.has(field))) setStep(2);
    else setStep(3);
  ...
  function validateTransaction() {
    ...
    for (const field of ["coreCharge", "exchangeFee", "freight"] as const) {
  ...
  return <div className="price-check-form-shell" data-mobile-aog-suppress>
    <ol className="price-check-progress" aria-label="Price Check progress">
      {["Part / transaction", "Document", "Requester"].map((label, index) => {
        const number = index + 1;
        return <li key={label} aria-current={step === number ? "step" : undefined} className={step >= number ? "is-active" : ""}>
    ...
    <form className="price-check-form" noValidate onSubmit={submit} onChange={() => {
      ...
      {step === 1 && <section aria-labelledby="price-check-step-1">
        <div className="price-check-step-heading"><span>STEP 01</span><h2 id="price-check-step-1">Part and transaction</h2><p>Share the quoted or purchased transaction as it was presented. No market conclusion is calculated at submission.</p></div>
        <div className="pc-field-grid">
          ...
          <label htmlFor="price-check-quantity"><FieldLabel htmlFor="price-check-quantity" required>Quantity</FieldLabel><input id="price-check-quantity" inputMode="decimal" value={values.quantity} onChange={(e) => update("quantity", e.target.value)} maxLength={16} {...errorProps("quantity")} />{fieldError("quantity")}</label>
        </div>
        ...
        <div className="pc-field-grid">
          <label htmlFor="price-check-transactionType"><FieldLabel htmlFor="price-check-transactionType" required>Transaction type</FieldLabel><select id="price-check-transactionType" value={values.transactionType} onChange={(e) => update("transactionType", e.target.value as FormValues["transactionType"])}>{Object.entries(transactionLabels).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <label htmlFor="price-check-conditionCode"><FieldLabel htmlFor="price-check-conditionCode" required>Condition</FieldLabel><select id="price-check-conditionCode" value={values.conditionCode} onChange={(e) => update("conditionCode", e.target.value as FormValues["conditionCode"])}>{conditionCodes.map((value) => <option key={value} value={value}>{conditionLabels[value]}</option>)}</select></label>
        </div>
        ...
        <fieldset className="pc-choice-group"><legend>Aircraft on ground (AOG) <span aria-hidden="true">*</span></legend><div>{[{value:false,label:"No"},{value:true,label:"Yes — active AOG"}].map((option) => <label key={String(option.value)}><input type="radio" name="pc-aog" checked={values.aog === option.value} onChange={() => update("aog", option.value)} /><span>{option.label}</span></label>)}</div></fieldset>
        {values.aog && <div className="pc-aog-notice" role="note"><strong>Active AOG requirement</strong><p>Submit the Price Check if useful, but call or WhatsApp Civilon&apos;s monitored AOG desk for immediate sourcing coordination. Price Check itself is not an immediate-result service.</p><div><CallAogAction className="button button-primary" source_page="/price-check">Call +1 909 344 4444</CallAogAction><WhatsAppAogAction className="button button-whatsapp" source_page="/price-check">WhatsApp AOG</WhatsAppAogAction></div></div>}
        ...
        <div className="pc-step-actions"><button type="button" className="pc-next" onClick={() => { if (validateTransaction()) { setStep(2); window.scrollTo({top:document.getElementById("price-check-form")?.offsetTop ?? 0,behavior:"smooth"}); } }}>Continue to document <span aria-hidden="true">→</span></button></div>
      </section>}
      ...
      {step === 2 && <section aria-labelledby="price-check-step-2">
        ...
        <div className="pc-document-notice" role="note"><strong>Commercially sensitive information</strong><p>Civilon stores uploaded files privately and uses them only to provide the requested Price Check under the current service-processing policy. Uploaded documents may be processed using automated extraction tools to help Civilon identify transaction details. Extracted information is reviewed by Civilon before it is used in the Price Check. Do not upload information unnecessary for the request. Public privacy wording is pending final legal review.</p></div>
      ...
      {step === 3 && <section aria-labelledby="price-check-step-3">
        <div className="price-check-step-heading"><span>STEP 03</span><h2 id="price-check-step-3">Requester</h2><p>Provide the business contact Civilon should use to review this Price Check.</p></div>
