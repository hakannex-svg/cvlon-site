    ...
    <form className="price-check-form" noValidate onSubmit={submit} onChange={() => {
      ...
      <details className="pc-disclosure" open={disclosures.transaction} onToggle={(e) => setDisclosure("transaction", e.currentTarget.open)}>
        ...
        <div className="pc-disclosure-body">
          ...
          <fieldset className="pc-choice-group"><legend>Aircraft on ground (AOG)</legend><div>{[{value:false,label:"No"},{value:true,label:"Yes — active AOG"}].map((option) => <label key={String(option.value)}><input type="radio" name="pc-aog" checked={values.aog === option.value} onChange={() => update("aog", option.value)} /><span>{option.label}</span></label>)}</div></fieldset>
          {values.aog && <div className="pc-aog-notice" role="note"><strong>Active AOG requirement</strong><p>Submit the Price Check if useful, but call or WhatsApp Civilon&apos;s monitored AOG desk for immediate sourcing coordination. Price Check itself is not an immediate-result service. A phone number is required below for an AOG request.</p><div><CallAogAction className="button button-primary" source_page="/price-check">Call +1 909 344 4444</CallAogAction><WhatsAppAogAction className="button button-whatsapp" source_page="/price-check">WhatsApp AOG</WhatsAppAogAction></div></div>}
