...
export function AdminResultWorkspace(props: {
  ...
  return <section className="admin-analysis-workspace admin-result-workspace" aria-labelledby="result-workspace-title">
    <header className="admin-analysis-heading"><div><p className="admin-eyebrow">Customer result</p><h2 id="result-workspace-title">Prepare the reviewed result</h2><p>This is what the customer will see. Confirm classification, range or limitations, confidence, factors, explanation, and disclaimer. Approval and sending are separate actions.</p><a className="admin-guide-link" href="/admin/help#result">What is this?</a></div><span className="admin-engine-label">Human approved</span></header>
    <div className="admin-result-layout">
      ...
      <section className="admin-panel admin-delivery-status" aria-labelledby="delivery-title"><div className="admin-panel-heading"><div><p className="admin-eyebrow">Send & delivery</p><h3 id="delivery-title">Secure access status</h3><p>Sending queues the secure result notification. Confirm the delivery status here.</p></div></div>{props.delivery.length ? <ul>{props.delivery.map((item, index) => <li key={index}><strong>{item.state === "succeeded" ? "Delivery successful" : item.state === "failed" ? "Delivery failed - retry available" : item.state === "pending" ? "Approved - delivery pending" : item.state.replaceAll("_", " ")}</strong><span>Attempts: {item.attemptCount}</span>{item.failureCode && <small>{item.failureCode}</small>}</li>)}</ul> : <p className="admin-muted">No delivery has been queued.</p>}</section>
    </div>

    <section className="admin-panel admin-ai-explanation" aria-labelledby="ai-explanation-title">
      <div className="admin-panel-heading"><div><p className="admin-eyebrow">AI explanation · Optional</p><h3 id="ai-explanation-title" ref={aiHeadingRef} tabIndex={-1}>Draft explanation with AI</h3><p>AI-assisted draft; requires staff review. Apply only after reviewing it against the deterministic analysis.</p></div><a className="admin-guide-link" href="/admin/help#result">Guide →</a><span className={`admin-ai-state state-${aiStatus.toLowerCase()}`}>{aiStatus.replaceAll("_", " ")}</span></div>
