            ...
            <fieldset className="needed-by-fields">
              ...
              {neededByMode === "specific" && <div className="needed-by-specific">
                ...
                <fieldset className="time-select-group" aria-describedby={neededByError ? fieldId("needed-by-error") : fieldId("needed-by-help")}>
                  ...
                  <div>
                    <label className="sr-only" htmlFor={fieldId("required-by-hour")}>Hour</label><select id={fieldId("required-by-hour")} name="requiredByHour" value={neededByHour} aria-required="true" aria-invalid={Boolean(neededByError && !neededByHour)} onChange={(event) => { setNeededByHour(event.target.value); setNeededByError(""); }}><option value="">Hour</option>{Array.from({ length: 12 }, (_, index) => String(index + 1).padStart(2, "0")).map((hour) => <option key={hour}>{hour}</option>)}</select>
                    <label className="sr-only" htmlFor={fieldId("required-by-minute")}>Minute</label><select id={fieldId("required-by-minute")} name="requiredByMinute" value={neededByMinute} aria-required="true" aria-invalid={Boolean(neededByError && !neededByMinute)} onChange={(event) => { setNeededByMinute(event.target.value); setNeededByError(""); }}><option value="">Min</option>{FIVE_MINUTE_OPTIONS.map((minute) => <option key={minute}>{minute}</option>)}</select>
                    <label className="sr-only" htmlFor={fieldId("required-by-period")}>AM or PM</label><select id={fieldId("required-by-period")} name="requiredByPeriod" value={neededByPeriod} aria-required="true" aria-invalid={Boolean(neededByError && !neededByPeriod)} onChange={(event) => { setNeededByPeriod(event.target.value as TimePeriod); setNeededByError(""); }}><option value="">AM/PM</option><option>AM</option><option>PM</option></select>
                  </div>
                </fieldset>
