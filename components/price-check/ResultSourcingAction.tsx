...
export function ResultSourcingAction({ alreadyRequested = false }: { alreadyRequested?: boolean }) {
  ...
  return <section className="result-sourcing" aria-live="polite"><div><p>Need sourcing support?</p><h2>Get a Civilon Quote</h2><span>Civilon can follow up using the transaction context already reviewed.</span></div>{state === "done" ? <strong>Your sourcing request has been sent to the Civilon team.</strong> : <button type="button" disabled={state === "busy"} onClick={submit}>{state === "busy" ? "Sending…" : "Get a Civilon Quote"}</button>}{state === "error" && <p>We could not record the request. Contact Civilon for assistance.</p>}</section>;
}
