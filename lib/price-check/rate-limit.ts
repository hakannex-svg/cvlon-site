...
export function rateLimitKey(value: string) {
  return createHash("sha256").update(`civilon-price-check-rate:v1:${value}`).digest("hex");
}

export function consumePriceCheckAttempt(key: string, now = Date.now()) {
...
export function resetPriceCheckRateLimitForTests() {
  attempts.clear();
}
